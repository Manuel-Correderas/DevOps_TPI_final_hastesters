...
class Address(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "tipo", name="uq_address_user_tipo"),
    )
...
class CryptoWallet(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "red", name="uq_wallet_user_red"),
    )
...
class Product(Base):
    ...
    price: Mapped[int] = mapped_column(Integer)  # entero ARS o centavos
...
class CartItem(Base):
    ...
    seller: Mapped[str] = mapped_column(String(120))  # snapshot nombre vendedor
...
class Order(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), default="Entregado")
...
class Payment(Base):
    ...
    provider: Mapped[str] = mapped_column(String(30), default="MP")
